...
def get_algorithm(algorithm, dataset, writer):
    ...
    if algorithm == 'linucb':
        ...
        alg = NN(
            ...
            all_contexts = dataset.get_all_contexts(),
            all_rewards = dataset.get_all_rewards())
    elif algorithm == 'rofu':
        assert args.loss_type == 'mse'
        alg = ROFU(
            writer,
            generate_model_config(dataset.contexts_shape, dataset.n_arms),
...
def save_data(sim, regrets_dict, action_value, mean_value, suffix):
    ...
    for dataset_name in args.datasets:
        ...
        np.save(current_path + current_name + '.npy', {'regrets_dict': regrets_dict, 'all_args': all_args, 'ucb': np.array(action_value), 'mean_value': np.array(mean_value)})
        ...
        for dataset_name in args.datasets:
            ...
            for algorithm in args.algorithms:
                ...
                for t in range(args.timestep - dataset.n_arms * 3):
                    ...
                    current_evaluation_regrets.append(current_regret)
                    try:
                      current_action_value.append(alg.ucb)
                    except:
                      pass
